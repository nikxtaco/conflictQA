...
if __name__ == '__main__':
    ...
    test_data = build_zeroshot_prompt_strategyQA_with_singleSource_evidence(args.input_file, mode="explicit", evidence_order='para_conf')
    print(test_data)
    total_price = 0
    for idx, prompt in enumerate(tqdm(test_data[:20])):
        if prompt == "":
        ...
        #                                    user_input=prompt, model_name=args.model_name, temperature=0) # Edited: commented out
        
        results, updated_history, price = asyncio.run(prompt_chatgpt("You are a helpful assistant.", index=idx, save_path=args.output_file,
                                           user_input=prompt, model_name=args.model_name, temperature=0))
    
        print(f"Assistant Output: {results}")
